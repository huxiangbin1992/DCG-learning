...
class DCG(nn.Module):
    def __init__(self):
        ...
        self.main = nn.Sequential(
            nn.Linear(MNIST_dimension + tabular_dimension + (K-1) + reference_dimension, G_width),
            nn.ReLU(),
...
def train_generator(rep_index, Data):
    XA_image_train = Data['XA_image_train'].to(device)
    R_train = Data['R_train'].to(device)
    reference = Data['reference'].to(device)
    XA_image_validation = Data['XA_image_validation'].to(device)
    ...
    def Distribution_distence(XA_image_validation, reference_validation, R_validation, generator):
        generated = generator(XA_image_validation, reference_validation).detach().cpu().numpy()
    ...
    XA_image_train_repeated = XA_image_train.repeat(1, J).view(-1, MNIST_dimension + tabular_dimension + (K-1))
    ...
    for epoch in range(epoch_size):
        ...
        d_loss_real = discriminator(XA_image_train[indices,:], R_train[indices,:])
        d_loss_real = d_loss_real.mean()
        d_loss_real.backward(one)
        
        fake_images = generator(XA_image_train_repeated[repeated_indices,:], reference[repeated_indices,:]).to(device)   
        d_loss_fake = discriminator(XA_image_train_repeated[repeated_indices,:], fake_images)
        d_loss_fake = d_loss_fake.mean()
        d_loss_fake.backward(minus_one)
        
        d_loss_penalty = dis_gradient(discriminator,XA_image_train[indices,:], R_train[indices,:])
        ...
        generator.zero_grad()
        # Select indices
        perm = torch.randperm(XA_image_train.size(0), device=device)  
        indices = perm[: 2 * batch_size]
        repeated_indices = (J * indices[:, None] + offsets[None, :]).view(-1)
        # Train generator
        fake_images = generator(XA_image_train_repeated[repeated_indices,:], reference[repeated_indices,:]).to(device)
        g_loss = discriminator(XA_image_train_repeated[repeated_indices,:], fake_images)
        g_loss = g_loss.mean()
        g_loss.backward()
        generator_optimizer.step()
        # time3 = time.time()
        
        WS_D = Distribution_distence(XA_image_validation, reference_validation, R_validation, generator)
        # time4 = time.time()
...
def generate_data(rep_index): 
    ...
    XA_image = np.hstack((indicator_A, XX_image))
    XA_image_train = torch.tensor(XA_image[:train_size], dtype=torch.float32)
    R_train = torch.tensor(R[:train_size], dtype=torch.float32)
    
    XA_image_validation = torch.tensor(XA_image[train_size:train_size+validation_size], dtype=torch.float32)
    R_validation = torch.tensor(R[train_size:train_size+validation_size], dtype=torch.float32)
    
    X_test = torch.tensor(X_1[-test_size:, :], dtype=torch.float32)
    X_image_test = torch.tensor(XX_image[-test_size:, :], dtype=torch.float32)
    MNIST_label_test = torch.tensor(MNIST_label[-test_size:, :], dtype=torch.float32)

    reference =  torch.tensor( np.random.uniform(-1, 1, size=(XA_image_train.shape[0]*J,reference_dimension) ) , dtype=torch.float32)
    reference_validation = torch.tensor( np.random.uniform(-1, 1, size=(validation_size,reference_dimension) ) , dtype=torch.float32)
    return {'XA_image_train':XA_image_train, 'R_train':R_train, 'reference':reference, 'XA_image_validation':XA_image_validation, 
            'R_validation':R_validation, 'reference_validation':reference_validation, 
            'X_test':X_test, 'X_image_test':X_image_test, 'MNIST_label_test':MNIST_label_test}
...
def test_ITR(Data,W_Gan):
    ...
    X_image_test = Data['X_image_test'].to(device)
    ...
    XA1_test = torch.cat((torch.full((test_size, 2), 0, device=device), X_image_test), dim=1)
    XA2_test = torch.cat((torch.full((test_size, 1), 1, device=device), torch.full((test_size, 1), 0, device=device), X_image_test), dim=1)
    XA3_test = torch.cat((torch.full((test_size, 1), 0, device=device), torch.full((test_size, 1), 1, device=device), X_image_test), dim=1)
