...
def generate_data(rep_index): 
    ...
    W_test = torch.tensor(W[-test_size:, :], dtype=torch.float32)
    ...
    return {'X_train':X_train, 'R_train':R_train, 'reference':reference, 'X_validation':X_validation, 'R_validation':R_validation, 
            'reference_validation':reference_validation, 'X_test':X_test, 'W_test':W_test}
...
def test_ITR(Data,W_Gan):
    ...
    W_test = Data['W_test'][:, -4:].to(device)
    mu = torch.exp(W_test.to(device) ).mean(axis=1).reshape(-1, 1)
    ER_1 = mu + 2*torch.sum(W_test, dim=1).view(-1, 1)
    ER_2 = mu + torch.sum( torch.sqrt(torch.abs(W_test)) , dim=1).view(-1, 1)/2
    ER_3 = mu + torch.sum(W_test**2, dim=1).view(-1, 1)/2
    ER_4 = mu + torch.sum(torch.cos(W_test), dim=1).view(-1, 1)/2
    ER = torch.cat((ER_1, ER_2, ER_3, ER_4), dim=1)
