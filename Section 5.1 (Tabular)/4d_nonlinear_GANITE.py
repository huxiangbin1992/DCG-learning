...
def test_ITR(test_x,y_all_hat):
    mu = np.exp(test_x).mean(axis=1).reshape(-1, 1)
    ER_1 = mu + 2*np.sum(test_x, axis=1).reshape(-1, 1) 
    ER_2 = mu + np.sum( np.sqrt(np.abs(test_x)) , axis=1) .reshape(-1, 1)/2
    ER_3 = mu + np.sum(test_x**2, axis=1).reshape(-1, 1)/2
    ER_4 = mu + np.sum(np.cos(test_x), axis=1).reshape(-1, 1)/2
