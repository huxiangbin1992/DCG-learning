...
def generate_data(rep_index): 
    ...
    epsilon_sd = (np.sqrt( np.sum(XX**2,axis=1) )/4).reshape(-1, 1)
    ...
    return {'X_train':X_train, 'R_train':R_train, 'reference':reference, 'X_validation':X_validation, 'R_validation':R_validation, 
            'reference_validation':reference_validation, 'X_test':X_test}
...
def true_value(Mon_size):
    ...
    XX = np.hstack((X_1, X_2))
    E_sigma = np.mean( np.sqrt(np.sum(XX**2, axis=1)) /4)
    mu = np.exp(XX).mean(axis=1).reshape(-1, 1)
    gamma_1 = 2*np.sum(XX, axis=1).reshape(-1, 1) 
    gamma_2 = np.sum( np.sqrt(np.abs(XX)) , axis=1) .reshape(-1, 1)/2
    gamma_3 = np.sum(XX**2, axis=1).reshape(-1, 1)/2
...
def test_ITR(Data,W_Gan):
    ...
    mu = torch.exp(X_test.to(device) ).mean(axis=1).reshape(-1, 1)
    ER_1 = mu + 2*torch.sum(X_test, dim=1).view(-1, 1)
    ER_2 = mu + torch.sum( torch.sqrt(torch.abs(X_test)) , dim=1).view(-1, 1)/2
    ER_3 = mu + torch.sum(X_test**2, dim=1).view(-1, 1)/2
    ER_4 = mu + torch.sum(torch.cos(X_test), dim=1).view(-1, 1)/2
