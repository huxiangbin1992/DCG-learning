...
qtest <- function(X, W, A, Y, fit, result) {
  n <- nrow(X)
  p <- ncol(X)
...
adtest <- function(X, W, A, Y, fit, result) {
  n <- nrow(X)
  p <- ncol(X)
  ITR = TITR(W)
  prop_test = predict(fit, X, s = "lambda.min", type = "response")[cbind(1:n,A,1)]
...
sadtest <- function(X, W, A, Y, fit, result) {
  n <- nrow(X)
  p <- ncol(X)
  ITR = TITR(W)
  prop_test = predict(fit, X, s = "lambda.min", type = "response")[cbind(1:n,A,1)]
...
radtest <- function(X, W, A, Y, fit, result) {
  n <- nrow(X)
  p <- ncol(X)
  ITR = TITR(W)
  prop_test = predict(fit, X, s = "lambda.min", type = "response")[cbind(1:n,A,1)]
...
generate_data <- function (rep_index){
  set.seed(rep_index)
  W_1 = matrix ( rtruncnorm(sample_size * 2, a = -3, b = 3, mean = 0, sd = 1), sample_size, 2)
  W_2 = matrix ( runif(sample_size * 2, min = -1, max = 1), sample_size, 2)
  W=cbind(W_1,W_2)
  mu = apply(exp(W),1,mean)
  PA = cbind( abs(W[,1]), 2*abs(W[,2]), 3*abs(W[,3]), 4*abs(W[,4]) )
  ...
  epsilon_sd = sqrt(apply(W^2,1,sum))/4
  for (i in 1:sample_size){epsilon[i] = rtruncnorm(1, a = -3*epsilon_sd[i], b = 3*epsilon_sd[i], mean = 0, sd = epsilon_sd[i])}

  Y[which(A==1)] = (mu + 2 * apply( W, 1, sum) + epsilon)[which(A==1)]
  Y[which(A==2)] = (mu + apply( sqrt(abs(W)), 1, sum)/2 + epsilon)[which(A==2)]
  Y[which(A==3)] = (mu + apply(W^2, 1, sum)/2 + epsilon)[which(A==3)]
  Y[which(A==4)] = (mu + apply(cos(W),1,sum)/2 + epsilon)[which(A==4)]
  # normalize the reward Y
  # min_Y= min(Y)
  # max_Y = max(Y)
  # Y_normalized = (Y - min_Y) / (max_Y - min_Y)
  X = matrix(0, sample_size, 20)
  X[,c(1,6,11,16)] = W
  X[,c(2,7,12,17)] = 2*W
  X[,c(3,8,13,18)] = -W
  X[,c(4,9,14,19)] = W^3
  X[,c(5,10,15,20)] = sin(W)
  A_train = A[1:train_size]
  X_train = X[1:train_size,]
  Y_train = Y[1:train_size]
  A_test = A[(train_size+1):sample_size]
  X_test = X[(train_size+1):sample_size,]
  W_test = W[(train_size+1):sample_size,]
  Y_test = Y[(train_size+1):sample_size]
  return(list( A_train=A_train, X_train=X_train, Y_train=Y_train, A_test=A_test, X_test=X_test, W_test=W_test, Y_test=Y_test))
}

TITR <- function(W){
  mu = apply(exp(W),1,mean)
  EY1 = mu + 2 * apply( W, 1, sum)
  EY2 = mu + apply( sqrt(abs(W)), 1, sum)/2
  EY3 = mu + apply(W^2, 1, sum)/2
  EY4 = mu + apply(cos(W),1,sum)/2
...
Tvalue <- function(Mon_size){
  set.seed(0)
  W_1 = matrix ( rtruncnorm(Mon_size * 2, a = -3, b = 3, mean = 0, sd = 1), Mon_size, 2)
  W_2 = matrix ( runif(Mon_size * 2, min = -1, max = 1), Mon_size, 2)
  W=cbind(W_1,W_2)
  mu = apply(exp(W),1,mean)
  gamma_1 = mu + 2 * apply( W, 1, sum)
  gamma_2 = mu + apply( sqrt(abs(W)), 1, sum)/2
  gamma_3 = mu + apply(W^2, 1, sum)/2
  gamma_4 = mu + apply(cos(W),1,sum)/2
...
for(rep_index in 1:num_rep){
  ...
  qtest_result = qtest(data$X_test, data$W_test, data$A_test, data$Y_test, Multilogistic.fit, ql_result)
  ad_result = adlearn(data$X_train, data$A_train, data$Y_train, prob_A_train, K)
  adtest_result = adtest(data$X_test, data$W_test, data$A_test, data$Y_test, Multilogistic.fit, ad_result)
  ...
  radtest_result = radtest(data$X_test, data$W_test, data$A_test, data$Y_test, Multilogistic.fit, rad_result)
  sad_result = sabdlearn(data$X_train, data$A_train, data$Y_train, prob_A_train, K)
  sadtest_result = sadtest(data$X_test, data$W_test, data$A_test, data$Y_test, Multilogistic.fit, sad_result)
