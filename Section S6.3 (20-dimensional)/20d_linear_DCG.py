...
def generate_data(rep_index): 
    ...
    A = np.random.choice([1, 2, 3, 4], size=(total_size, 1), replace=True) # generate A from unif 1,2,3,4
    ...
    beta_1 = np.reshape( np.array([np.repeat(1.0,5), np.repeat(1.0,5), np.repeat(1.0,5), np.repeat(1.0,5)]), 20)
    beta_2 = np.reshape( np.array([np.repeat(1.0,5), np.repeat(1.0,5), np.repeat(-1.0,5), np.repeat(-1.0,5)]), 20)
    beta_3 = np.reshape( np.array([np.repeat(1.0,5), np.repeat(-1.0,5), np.repeat(1.0,5), np.repeat(-1.0,5)]), 20)
    beta_4 = np.reshape( np.array([np.repeat(-1.0,5), np.repeat(-1.0,5), np.repeat(1.0,5), np.repeat(1.0,5)]), 20)
    epsilon = truncnorm(-3, 3, 0, 1).rvs(total_size).reshape(-1, 1)
    mu = 2 + np.sum( XX, axis=1 ).reshape(-1, 1)
    gamma_1 = np.dot(XX,beta_1).reshape(-1, 1)
    gamma_2 = np.dot(XX,beta_2).reshape(-1, 1)
    gamma_3 = np.dot(XX,beta_3).reshape(-1, 1)
    gamma_4 = np.dot(XX,beta_4).reshape(-1, 1)
...
def true_value(mon_size):
    ...
    beta_3 = np.reshape( np.array([np.repeat(1.0,5), np.repeat(-1.0,5), np.repeat(1.0,5), np.repeat(-1.0,5)]), 20)
    beta_4 = np.reshape( np.array([np.repeat(-1.0,5), np.repeat(-1.0,5), np.repeat(1.0,5), np.repeat(1.0,5)]), 20)
    mu = 2 + np.sum( XX, axis=1 )
    mean_1 = (np.dot(XX,beta_1) + mu).reshape(-1, 1)
    mean_2 = (np.dot(XX,beta_2) + mu).reshape(-1, 1)
    mean_3 = (np.dot(XX,beta_3) + mu).reshape(-1, 1)
    mean_4 = (np.dot(XX,beta_4) + mu).reshape(-1, 1)
    Mean = np.hstack((mean_1, mean_2, mean_3, mean_4))
    value = np.max(Mean, axis=1).mean()
    return value

##################### Test Procedure #########################
def test_ITR(Data,W_Gan):
    X_test = Data['X_test'][:, -cov_dimension:].to(device)
    beta_1 = torch.reshape(torch.tensor([1.0] * 5 + [1.0] * 5 + [1.0] * 5 + [1.0] * 5), (20,)).to(device) 
    beta_2 = torch.reshape(torch.tensor([1.0] * 5 + [1.0] * 5 + [-1.0] * 5 + [-1.0] * 5), (20,)).to(device) 
    beta_3 = torch.reshape(torch.tensor([1.0] * 5 + [-1.0] * 5 + [1.0] * 5 + [-1.0] * 5), (20,)).to(device) 
    beta_4 = torch.reshape(torch.tensor([-1.0] * 5 + [-1.0] * 5 + [1.0] * 5 + [1.0] * 5), (20,)).to(device) 
    mu = 2 + torch.sum(X_test, dim=1) 
    ER1 = torch.matmul(X_test,beta_1) + mu
    ER2 = torch.matmul(X_test,beta_2) + mu
    ER3 = torch.matmul(X_test,beta_3) + mu
    ER4 = torch.matmul(X_test,beta_4) + mu
...
T_value = true_value(5000000)
T_Cvar75 = T_value - 0.4195317
T_Cvar50 = T_value - 0.7911568
T_Cvar25 = T_value - 1.258595
T_Cvar10 = T_value - 1.72914
