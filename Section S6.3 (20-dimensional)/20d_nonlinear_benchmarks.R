...
generate_data <- function (rep_index){
  ...
  epsilon_sd = sqrt(apply(X^2,1,sum))/10
  for (i in 1:sample_size){epsilon[i] = rtruncnorm(1, a = -3*epsilon_sd[i], b = 3*epsilon_sd[i], mean = 0, sd = epsilon_sd[i])}
  
  Y[which(A==1)] = (apply( X, 1, sum) + epsilon)[which(A==1)]
  Y[which(A==2)] = (apply( sqrt(abs(X)), 1, sum) + epsilon)[which(A==2)]
  Y[which(A==3)] = (apply(X^2, 1, sum) + epsilon)[which(A==3)]
  Y[which(A==4)] = (apply(cos(X),1,sum) + epsilon)[which(A==4)]
...
TITR <- function(X){
  EY1 = apply( X, 1, sum)
  EY2 = apply( sqrt(abs(X)), 1, sum)
  EY3 = apply(X^2, 1, sum)
  EY4 = apply(cos(X),1,sum)
...
Tvalue <- function(Mon_size){
  ...
  X=cbind(X_1,X_2)
  gamma_1 = apply( X, 1, sum)
  gamma_2 = apply( sqrt(abs(X)), 1, sum)
  gamma_3 = apply(X^2, 1, sum)
  gamma_4 = apply(cos(X),1,sum)
