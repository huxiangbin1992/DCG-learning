...
generate_data <- function (rep_index){
  set.seed(rep_index)
  A = sample(1:K, sample_size, replace = T)
  X_1 = matrix ( rtruncnorm(sample_size * 10, a = -3, b = 3, mean = 0, sd = 1), sample_size, 10)
  X_2 = matrix ( runif(sample_size * 10, min = -1, max = 1), sample_size, 10)
  X=cbind(X_1,X_2)
  mu = 2 + apply(X,1,sum)
  epsilon = rtruncnorm(sample_size, a = -3, b = 3, mean = 0, sd = 1)
  Y = rep(0,sample_size)
  Y[which(A == 1)] = (mu + X%*%beta_1 + epsilon)[which(A == 1)]
  Y[which(A == 2)] = (mu + X%*%beta_2 + epsilon)[which(A == 2)]
  Y[which(A == 3)] = (mu + X%*%beta_3 + epsilon)[which(A == 3)]
  Y[which(A == 4)] = (mu + X%*%beta_4 + epsilon)[which(A == 4)]
...
TITR <- function(X){
  EY1 = 2 + apply(X,1,sum) + X%*%beta_1
  EY2 = 2 + apply(X,1,sum) + X%*%beta_2
  EY3 = 2 + apply(X,1,sum) + X%*%beta_3
  EY4 = 2 + apply(X,1,sum) + X%*%beta_4
...
Tvalue <- function(Mon_size){
  set.seed(0)
  A = sample(1:K, Mon_size, replace = T)
  X_1 = matrix ( rtruncnorm(Mon_size * 10, a = -3, b = 3, mean = 0, sd = 1), Mon_size, 10)
  X_2 = matrix ( runif(Mon_size * 10, min = -1, max = 1), Mon_size, 10)
  X=cbind(X_1,X_2)
  gamma_1 = X%*%beta_1
  gamma_2 = X%*%beta_2
  gamma_3 = X%*%beta_3
  ...
  value = mean(apply(gamma,1,max)) + 2
  return(value)
}
...
K = 4
beta_1 = c(rep(1,10),rep(1,10))
beta_2 = c(rep(1,10),rep(-1,10))
beta_3 = c(rep(1,5),rep(-1,5),rep(1,5),rep(-1,5))
beta_4 = c(rep(-1,10),rep(1,10))
